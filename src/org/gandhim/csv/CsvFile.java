...
public class CsvFile implements Constants {
    ...
    public void sematictodb(){
        File org_file = new File(SEMANTIC_FILE);
        try
        {
    ...
    public void genSemanticData(){
        ...
        for (int j=0;j < date.length; j++){
            String trainData = TRAIN_PATH+org_file.getName()+date[j]+"d";
        ...
        try {
            ...
            for (NewsType newsType : newsTypes) {
                for (String feature: features) {
                    ...
                    for (Lda lda : ldas) {
                        File file = new File(SEMANTIC_PATH+"semanticF"+feature+"T"+lda.getFileType()+".csv");
            ...
            for (NewsType newsType : newsTypes) {
                for (String feature: features) {
                    ...
                    for (Lda lda : ldas) {
                        ...
                        ArrayList<MarketResult> marketResults = ldaDao.getMarketResults(lda.getId());
                        for (MarketResult marketResult: marketResults) {
                            File file = new File(SEMANTIC_PATH+feature+"/"+lda.getFileType()+"_"+feature+"_"+marketResult.getDate()+"d");
                            FileWriter writer = new FileWriter(file,true);
